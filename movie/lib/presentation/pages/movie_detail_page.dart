...
import '../bloc/movie_recommendation/movie_recommendation_bloc.dart';
...
class _MovieDetailPageState extends State<MovieDetailPage> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() {
      context.read<MovieDetailBloc>().add(OnGetDetail(widget.id));
      context
          .read<MovieRecommendationBloc>()
          .add(OnGetRecommendation(widget.id));
      context.read<MovieWatchlistBloc>().add(WatchlistStatus(widget.id));
    });
  }

  @override
  Widget build(BuildContext context) {
    final isListed = context.select<MovieWatchlistBloc, bool>((bloc) {
      if (bloc.state is MovieWatchlistIsWatchlist) {
        return (bloc.state as MovieWatchlistIsWatchlist).isListed;
      }
      return false;
    });
    return Scaffold(
      body: BlocBuilder<MovieDetailBloc, MovieDetailState>(
        builder: (context, state) {
          if (state is MovieDetailEmpty) {
            ...
            final movie = state.detailResult;
            return SafeArea(
              child: DetailContent(
                movie,
                isListed,
...
class DetailContent extends StatefulWidget {
  ...
  DetailContent(this.movie, this.isListedWatchlist, {super.key});
...
class _DetailContentState extends State<DetailContent> {
  ...
  Future addRemoveWatchlist() async {
    if (!widget.isListedWatchlist) {
      context.read<MovieWatchlistBloc>().add(AddWatchlist(widget.movie));
    } else {
      context.read<MovieWatchlistBloc>().add(RemoveWatchlist(widget.movie));
    }
